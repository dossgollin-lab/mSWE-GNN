...
from graph_creation import (
    create_mesh_dhydro,
    generate_random_polygon_with_dike,
    save_mesh,
)
...
def mesh_DEM_generator(
    ...
    DEM = np.array(
        [
            [noise([i / diff, j / diff]) for j in range(extent_min, diff)]
            for i in range(extent_min, diff)
        ]
    )
    slope = np.array(
        [
            [
                i * np.cos(slope_direction) + j * np.sin(slope_direction)
                for j in range(extent_min, diff)
            ]
            for i in range(extent_min, diff)
        ]
    )
    ...
    DEM = interpolate_variable(
        mesh.face_xy, grid_nodes, DEM.reshape(-1), method="nearest"
    )
...
def save_DEM(DEM, dst_file, node_coords=None):
    ...
    if node_coords is None:
        ...
        y_grid = np.array(
            [[(i + 0.5) for j in range(number_grids)] for i in range(number_grids)]
        )
        x_grid = y_grid.T
        xyz = np.array(
            [
                [x, y, z]
                for x, y, z in zip(
                    x_grid.reshape(-1, 1), y_grid.reshape(-1, 1), DEM.reshape(-1, 1)
                )
            ]
        ).squeeze()
    else:
...
def generate_hydrograph(
    ...
    time_x = np.linspace(
        weibull_max.ppf(0.01, shape), weibull_max.ppf(0.999, shape), time_steps
    )
...
def generate_random_hydrograph(
    seed, total_time, time_resolution, min_discharge=0, peak=(100, 250), shape=(0, 3)
):
    ...
    return generate_hydrograph(
        shape, peak_value, min_discharge, total_time, time_resolution
    )
...
def from_output_nc_to_txt(output_map, save_folder, seed):
    ...
    np.savetxt(
        os.path.join(save_folder, "WD", f"WD_{seed}.txt"), waterdepth, fmt="%1.4f"
    )
    np.savetxt(
        os.path.join(save_folder, "VX", f"VX_{seed}.txt"), velocity_x, fmt="%1.4f"
    )
    np.savetxt(
        os.path.join(save_folder, "VY", f"VY_{seed}.txt"), velocity_y, fmt="%1.4f"
    )
...
def run_simulations_mesh(
    ...
    noise_octave=(3, 8),
    DEM_multiplier=(1, 7),
    slope_multiplier=(0, 0.001),
    num_vertices_polygon=(20, 30),
    ...
    for sim in tqdm(range(start_sim, start_sim + n_sim)):
        sim = (
            sim + 999 if sim == 91 else sim
        )  # change simulation 26 since it doesn't work
        if polygon_file is None:
            # Generate random polygon for mesh creation
            generate_random_polygon_with_dike(
                save_polygon=True,
                avg_radius=100 * grid_size,
                irregularity=0.2,
                spikiness=0.08,
                seed=sim,
        ...
        # Generate DEM
        if DEM_file is None:
            mesh_DEM = mesh_DEM_generator(
                sim,
                mesh=mesh,
                noise_octave=noise_octave,
                DEM_multiplier=DEM_multiplier,
            ...
            mesh_DEM = interpolate_variable(
                mesh.face_xy, grid_nodes, DEM, method="linear"
            )
        ...
        if random_hydrograph:
            time_x, y = generate_random_hydrograph(
                sim,
        ...
        add_boundary_condition(
            boundary_condition_file, time_series, init_time, BC_type=2
        )
        ...
        shutil.copy(
            boundary_polygon_file, f"{save_folder}\\Geometry\\polygon_{sim}.pol"
        )
        simulation_stats.append(
            [sim, mesh.face_x.shape[0], simulated_time_hours, computation_time]
        )

    df = pd.DataFrame(
        simulation_stats,
        columns=["seed", "mesh_num_faces", "simulation_time[h]", "computation_time[s]"],
    )
    df.to_csv(
        f"{save_folder}\\overview.csv",
        mode="a",
        sep=",",
        index=False,
        header=not os.path.exists(f"{save_folder}\\overview.csv"),
    )
