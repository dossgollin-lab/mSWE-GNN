...
from database.graph_creation import graph_from_mesh, MultiscaleMesh, remove_ghost_cells
from utils.miscellaneous import add_null_time_start, WD_to_FAT, plot_line_with_deviation
from utils.miscellaneous import get_time_vector, get_mass_conservation_loss, get_rollout_loss, get_CSI, get_F1
...
class BasePlotMap(object):
    ...
    def _add_colorbar(self, ax=None, colorbar=True, logscale=False):
        ...
        if colorbar:
            decimals = 2
            if logscale:
                plt.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=self.kwargs['cmap']), 
...
class QuiverPlotMap(BasePlotMap):
    ...
    def quiver_plot(self, time_step, ax=None, colorbar=True):
        ...
        X = coordinates[:,0]
        Y = coordinates[:,1]
        quiver_kwargs = {key: self.kwargs[key] for key in self.kwargs.keys() - {'vmin', 'vmax'}}
        
        q = ax.quiver(X[self.mask], Y[self.mask], 
                      field_x[self.mask], field_y[self.mask], 
                      self.field_module[self.mask], 
                      scale_units='height', 
                      width=0.0035, scale=10,
                      **quiver_kwargs)
...
class DEMPlotMap(BasePlotMap):
    ...
    def _add_axes_info(self, ax, title=True, x_label=True, y_label=True):
        ...
        ax.set_xticks(np.linspace(self.pos.min(0)[0].round(-3), self.pos.max(0)[0].round(-3), num=5))
        ax.set_yticks(np.linspace(self.pos.min(0)[1].round(-3), self.pos.max(0)[1].round(-3), num=5))
...
def plot_rollout_diff_in_time_all(diff_rollout, temporal_res, type_loss='RMSE', 
    ...
    lin_V = plot_rollout_diff_in_time_var(
        ...
        time_start=time_start, ax=ax2, label='|q|', c='purple')
    lns = lns + lin_V
...
def plot_breach_distribution(dataset, ax=None, with_label=True, **plt_kwargs):
    ...
    assert dataset[0].mesh.num_nodes == dataset[1].mesh.num_nodes, "All simulations must have the same mesh"
    ...
    breach_coordinates = np.array([[data.mesh.face_xy[node.item()] for node in data.node_BC] for data in dataset]).squeeze()

    plt.scatter(breach_coordinates[:,0], breach_coordinates[:,1], s=80, marker='X', zorder=3, **plt_kwargs)
...
class PlotRollout():
    ...
    **temporal_test_dataset_parameters: dict,    parameters for temporal dataset
    ...
    def __init__(self, model, dataset, scalers=None, type_loss='RMSE', **temporal_test_dataset_parameters):
        ...
        temporal_dataset = to_temporal(dataset, rollout_steps=-1, **temporal_test_dataset_parameters)[0]
    ...
    def mesh_scale_plot(self, scale):
        ...
        real_rollout = separate_multiscale_node_features(self.real_rollout, self.dataset.node_ptr)[scale]
        diff_rollout = separate_multiscale_node_features(self.diff_rollout, self.dataset.node_ptr)[scale]
    ...
    def _plot_DEM(self, ax):
        ...
        self.DEMPlot._add_breach_location(ax=ax, breach_coordinates=self.breach_coordinates)
    ...
    def _get_VQuiverPlots(self, real_rollout, predicted_rollout, diff_rollout, input_water):
        # Vector velocity
        self.real_V = QuiverPlotMap(real_rollout[:,1,:], real_rollout[:,2,:], 
            **self.default_temporal_plot_kwargs, scaler=self.scalers['V_scaler'], 
            cmap=V_color, vmax=self.V_max)

        self.predicted_V = QuiverPlotMap(predicted_rollout[:,1,:], predicted_rollout[:,2,:], 
            **self.default_temporal_plot_kwargs, scaler=self.scalers['V_scaler'], 
            cmap=V_color, vmax=self.V_max)
            
        self.difference_V = QuiverPlotMap(diff_rollout[:,1,:], diff_rollout[:,2,:], 
            **self.default_temporal_plot_kwargs, scaler=self.scalers['V_scaler'], 
            difference_plot=True, vmin=self.min_diff_V, vmax=self.max_diff_V)
            
        self.init_V = QuiverPlotMap(input_water[:,1,:], input_water[:,2,:],
            **self.default_plot_kwargs|{'time_start': -1, 'temporal_res':self.temporal_res}, 
            cmap=V_color, vmax=self.V_max)
    ...
    def _get_FATPlots(self, real_rollout, predicted_rollout):
        ...
        self.pred_FATPlot = BasePlotMap(predicted_FAT, **self.default_plot_kwargs, cmap=FAT_color)
        self.real_FATPlot = BasePlotMap(real_FAT, **self.default_plot_kwargs, cmap=FAT_color)
    ...
    def plot_BC(self, ax=None):
        ...
        if isinstance(self.dataset.mesh, MultiscaleMesh):
            inflow_nodes = (self.dataset.BC[:1].T * self.dataset.edge_BC_length[:1]).T  # only finest scale
        else:
        ...
        [plot_line_with_deviation(time_vector, inflow, ax=ax, label=f'{type_BC_dict[type_BC]}') for inflow in inflow_nodes.cpu()]
        ax.set_xlabel('Time [h]')
    ...
    def explore_multiscale_rollout(self, time_step=-1, variable='WD', logscale=False):
        ...
        assert isinstance(self.dataset.mesh, MultiscaleMesh), "This function only works for multiscale meshes"
    ...
    def compare_Froude(self, time_step):
        ...
        real_FrPlot = TemporalPlotMap(self.real_fr, 
            **self.default_temporal_plot_kwargs, cmap=V_color, vmin=0)
    ...
    def create_multiscale_video(self, variable='WD', interval=200, blit=False, **anim_kwargs):
        ...
        for i in range(self.num_scales):
            ...
            if i==0:
                ...
                self.DEMPlot._add_breach_location(ax=axs[i,0], breach_coordinates=self.breach_coordinates)
    ...
    def _plot_mass_conservation(self, normalized=True, with_cum=True, ax=None):
        ...
        mass_loss = get_mass_conservation_loss(self.predicted_rollout[self.dataset.node_ptr[0]:self.dataset.node_ptr[1]], self.dataset).cpu()*1e6
        mass_loss = add_null_time_start(self.time_start+1, mass_loss)

        inflow_volume = torch.tensor([get_inflow_volume(self.dataset, self.dataset.BC[:,t:t+2].mean(1)) for t in range(mass_loss.shape[0]-1)])
        ...
        if with_cum:
            ...
            plot_line_with_deviation(self.time_vector, mass_loss, label=r'Per $\Delta$t')
